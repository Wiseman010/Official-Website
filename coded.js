...
// s = selector string
// Usage: const element = $(selector);
// Example:
// $('.my-class').style.color = 'red';
...
const STORAGE_KEY = "NOROM_coded_content";
...
function makeEditor(id, mode) {
    ...
    const ed = ace.edit(id, {
        theme: "ace/theme/github-dark",
        mode, tabSize: 2, useSoftTab: true, wrap: true, showPrintMargin: false
    });
...
function buildwebScrdoc(withTests = false) {
...
function runWeb(withTests = false) {
    preview.srcdoc = buildwebScrdoc(withTests);
...
$("#openPreview")?.addEventListener("click", () => {
    const scr = buildwebScrdoc(false);
...
function loadProject(obj) {
    try {
        if ($("#assignment")) $("#assignment").value = obj.assignments || "";
...
function saveProject() {
     try {
        ...
        a.download = "NoROM-coded_project.json";
...
log("Coded.js initialized (HTML / CSS / JS).");